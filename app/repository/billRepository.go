...
func (b *billRepo) GetAll(period string) ([]models.Response, error) {
	...
	if err := db.Db.Table("bills").Select("room_id, name, pre_meter_value, new_meter_value, bill, period, additional_fee, remark, bills.id").Where("period = ?", period).Joins("left join students on students.id = bills.room_id").Scan(&res).Error; err != nil {
		return nil, err
	}
	return res, nil
}
...
func (b *billRepo) Update(bills []models.Bill) error {
	for _, bill := range bills {
		if err := db.Db.Model(&bill).Updates(bill).Error; err != nil {
			return err
		}
	}
	return nil
